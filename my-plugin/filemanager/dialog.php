...
if (isset ( $_POST ['submit'] )) {
	...
	if ($ftp) {
		...
		$thumbs_path = str_replace ( array (
				'/..',
				'..' 
		), '', $ftp_base_folder . $ftp_thumbs_dir . $rfm_subfolder );
...
<script src="js/modernizr.custom.js"></script>
	...
	if ($aviary_active) {
		if (! empty ( $_SERVER ['HTTPS'] ) && $_SERVER ['HTTPS'] !== 'off' || $_SERVER ['SERVER_PORT'] == 443) {
			?>
		<script
	src="https://dme0ih8comzn4.cloudfront.net/imaging/v2/editor.js"></script>
	<?php }else{ ?>
...
<body>
	...
	<input type="hidden" id="lower_case"
		value="<?php echo $lower_case?"true":"false";?>" />
	<input type="hidden" id="show_folder_size"
		value="<?php echo $show_folder_size;?>" />
	<input type="hidden" id="add_time_to_img"
		value="<?php echo $add_time_to_img;?>" />
<?php if($upload_files){ ?>
<!-- uploader div start -->
	<div class="uploader">
			...
			<div class="tab-content">
				<div class="tab-pane active" id="tab1">
					<form action="dialog.php" method="post"
						...
						<input type="hidden" name="path" id="cur_path"
							value="<?php echo $cur_path?>" /> <input type="hidden"
							name="path_thumb" value="<?php echo $thumbs_path.$subdir?>" />
						<div class="fallback">
							...
							<br /> <input name="file" type="file" /> <input type="hidden"
								...
								type="hidden" name="type" value="<?php echo $type_param;?>" /> <input
								type="hidden" name="field_id" value="<?php echo $field_id;?>" />
							<input type="hidden" name="relative_url"
								value="<?php echo $return_relative_url;?>" /> <input
								type="hidden" name="popup" value="<?php echo $popup;?>" /> <input
								type="hidden" name="lang" value="<?php echo $lang;?>" /> <input
								type="hidden" name="filter" value="<?php echo $filter;?>" /> <input
								type="submit" name="submit" value="<?php echo trans('OK')?>" />
						</div>
					</form>
					...
					<div class='file-extension'><?php echo trans('Type_dir');?></div>
							<figcaption>
								<a href="javascript:void('')"
									...
									data-path="<?php echo $rfm_subfolder.$subdir.$file;?>"> <i
				...
				?>		<figure data-name="<?php echo $file ?>"
					...
					<div class="img-container">
								...
								<div
									...
									<div
										class="filetype <?php echo $file_array['extension'] ?> <?php if(in_array($file_array['extension'], $editable_text_file_exts)) echo 'edit-text-file-allowed' ?> <?php if(!$is_icon_thumb){ echo "hide"; }?>"><?php echo $file_array['extension'] ?></div>
									<div class="img-container-mini">
					<?php if($mini_src!=""){ ?>
					<img
