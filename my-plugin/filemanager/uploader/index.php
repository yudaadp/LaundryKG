 ...
 * descriptions.
 *
 * @param $juploadPhpSupportClass The
...
function handle_uploaded_files($juploadPhpSupportClass, $files) {
	return "<P>We are in the 'handle_uploaded_files' callback function, in the index.php script. To avoid double coding, we " . "just call the default behavior of the JUpload PHP class. Just replace this by your code...</P>" . $juploadPhpSupportClass->defaultAfterUploadManagement ();
	;
}
...
$appletParameters = array (
		...
		//
		'debugLevel' => 0 
) // 100 disables redirect after upload, so we keep it below. This still gives a lot of information, in case of problem.
;
...
$classParameters = array (
		...
		'destdir' => $path 
) // Where to store the files on the web
